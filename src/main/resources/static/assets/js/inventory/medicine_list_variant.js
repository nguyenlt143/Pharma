// Inventory Medicine List - Master-Detail Variant Grouping with Pagination and Bulk Actions
document.addEventListener('DOMContentLoaded', function() {
    ...
    function buildMainRowHTML(index, variant) {
        ...
        html += '</td>';
        html += '<td class="px-3 py-2 text-center font-semibold">' + variant.totalQty + ' ' + variant.unit + '</td>';
        ...
        if (variant.hasExpired) {
            html += '<span class="inline-flex items-center gap-1 text-xs bg-red-100 text-red-800 px-2 py-1 rounded-full" title="Có lô sắp hết hạn"><span class="material-icons-outlined text-xs">warning</span>Hết hạn</span>';
        }
        if (!variant.hasExpired && variant.hasExpiring) {
            html += '<span class="inline-flex items-center gap-1 text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded-full" title="Có lô sắp hết hạn"><span class="material-icons-outlined text-xs">error_outline</span>Sắp hết hạn</span>';
        }
    ...
    tableBody.addEventListener('change', e => {
        ...
        if (e.target && e.target.classList.contains('min-stock-input')) {
            ...
            const allBatches = Array.from(document.querySelectorAll('.batch-row[data-variant-id="' + variantId + '"]'));
            const inventoryIds = allBatches.map(row => row.dataset.inventoryId).filter(id => id);
            ...
            inventoryIds.forEach(inventoryId => {
                fetch('/inventory/api/inventory/' + inventoryId + '/min-stock', {
                    ...
                    if (!data || data.success !== true) {
                        console.error('Failed to save min stock');
                    }
                }).catch(err => {
